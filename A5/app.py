...
app.secret_key = "montana-secret-key" # Set a secret key used to sign and protect session cookies
...
def get_db():                          # Define a function that opens and returns a new database connection
    return mysql.connector.connect(    # Returns mysql connection from database
        host="10.2.3.157",             # Ip adress to database
        port=3306,                     # Mysql port (may be blocked or allowed by firewall)
        user="senad",                  # Mariadb user-name
        password="12345",              # Mariadb password for user
        database="MONTANA"             # Name of the database to use
    )
...
def admin_toggle_user():
    ...
    new_status = "disabled" if user["active"] == "enabled" else "enabled"
...
# Kunde sender melding til admin
...
def send_message():
    ...
    if not email or not subject or not message:
        return jsonify({"ok": False, "error": "Fyll inn alle feltene."})
    ...
    cur = con.cursor()
    cur.execute(
        "INSERT INTO messages (user_id, email, subject, message) VALUES (%s, %s, %s, %s)",
